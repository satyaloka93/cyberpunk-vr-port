            ...
            if (m_actionSet != XR_NULL_HANDLE) {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    if (i == 0) {
                        ...
                        // D-PAD SHIFT/CHORD. The modifier is EITHER the PSVR2 Triangle touch /
                        // Quest thumbrest OR the left stick click -- upstream 0.1.5 dropped the
                        // touch half, and it is kept because SteamVR's Sense remapper omits the
                        // buttons that made the chord reachable one-handed (see
                        // .okf/fixes/psvr2-triangle-dpad.md). Upstream's tunable "to the stop"
                        // threshold is taken as-is: a resting thumb must not step a list.
