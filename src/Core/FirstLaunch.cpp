...
extern "C" void ApplyFirstLaunchGameSettings() {
    ...
    // ...AND ONLY THEN this fork's reinstall preservation. Ordered after the cascade guard on
    // purpose: a drifted cascade is a correctness fault the two views cannot be left with, so it
    // outranks preserving tuned graphics. With no drift, a reinstall keeps the player's settings.
    ...
    if (!cascadeDrifted && HasPriorVrSettingsBackup(local)) {
