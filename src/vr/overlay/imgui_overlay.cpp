...
extern "C" unsigned long long CyberpunkVR_DebugMonoSubmits;
...
struct FrameContext {
    ...
    // The allocator is reusable only after this exact submission completes.
    UINT64 fenceValue = 0;
};
...
// Mode 2 / middle ground: before recording overlay N+1, wait for overlay N's signal. Because the
// signal is inserted on Cyberpunk's shared Present queue after the overlay draw, this permits one
// frame of useful CPU/GPU overlap without draining the current frame or allowing unbounded queue
// runahead. This is separate from each allocator/upload slot's exact reuse fence.
UINT64 g_previousOverlayFenceValue = 0;
UINT g_rtvDescriptorSize = 0;
UINT g_frameCount = 0;
DXGI_FORMAT g_rtvFormat = DXGI_FORMAT_UNKNOWN;
std::vector<FrameContext> g_frames;
// Dear ImGui's DX12 backend rotates upload buffers sequentially, independently of the
// swapchain's backbuffer index. Fence that ring separately from the command allocators.
std::vector<UINT64> g_imguiFrameFenceValues;
UINT g_nextImGuiFrameSlot = 0;
bool g_overlaySubmissionDisabled = false;
// Streamline rotates Present across worker threads. The old queue drain accidentally serialized
...
// Compact always-on diagnostic for performance test builds. Present rate includes frames
// generated by Streamline/DLSS-G; VRCAM snapshots are the useful approximation of native game
// render rate, while XR cycles show compositor-facing cadence. Keep this independent of the
// expensive node profiler so observing basic pacing does not materially change it.
bool g_perfOverlayVisible = true;
std::atomic<bool> g_middleGroundPacingActive{false};
std::atomic<double> g_previousOverlayWaitMs{0.0};
std::atomic<double> g_overlaySlotWaitMs{0.0};

struct PerfOverlayMetrics {
    LARGE_INTEGER frequency{};
    LARGE_INTEGER windowStart{};
    uint64_t presents = 0;
    uint64_t lastXrCycles = 0;
    uint64_t lastVrcamCopies = 0;
    float presentFps = 0.0f;
    float xrHz = 0.0f;
    float vrcamFps = 0.0f;
    uint32_t windowsSinceLog = 0;
};

PerfOverlayMetrics g_perfMetrics;

void DrawPerformanceOverlay() {
    LARGE_INTEGER now{};
    QueryPerformanceCounter(&now);
    if (g_perfMetrics.frequency.QuadPart == 0) {
        QueryPerformanceFrequency(&g_perfMetrics.frequency);
        g_perfMetrics.windowStart = now;
        g_perfMetrics.lastXrCycles = CyberpunkVR_DebugXrCycles;
        g_perfMetrics.lastVrcamCopies = CyberpunkVR_DebugStableCopies;
    }
    ++g_perfMetrics.presents;

    const double elapsed = static_cast<double>(now.QuadPart - g_perfMetrics.windowStart.QuadPart) /
        static_cast<double>(g_perfMetrics.frequency.QuadPart);
    if (elapsed >= 0.5) {
        const uint64_t xrCycles = CyberpunkVR_DebugXrCycles;
        const uint64_t vrcamCopies = CyberpunkVR_DebugStableCopies;
        g_perfMetrics.presentFps = static_cast<float>(static_cast<double>(g_perfMetrics.presents) / elapsed);
        g_perfMetrics.xrHz = static_cast<float>(static_cast<double>(xrCycles - g_perfMetrics.lastXrCycles) / elapsed);
        g_perfMetrics.vrcamFps = static_cast<float>(static_cast<double>(vrcamCopies - g_perfMetrics.lastVrcamCopies) / elapsed);
        g_perfMetrics.presents = 0;
        g_perfMetrics.lastXrCycles = xrCycles;
        g_perfMetrics.lastVrcamCopies = vrcamCopies;
        g_perfMetrics.windowStart = now;
    ...
    if (!g_perfOverlayVisible) return;

    const ImVec2 display = ImGui::GetIO().DisplaySize;
    ImGui::SetNextWindowPos(ImVec2(display.x * 0.5f, display.y * 0.035f),
    ...
    if (ImGui::Begin("VR performance##CyberpunkVRPort", nullptr, flags)) {
        const float fps = g_perfMetrics.presentFps;
        const ImVec4 fpsColor = fps >= 80.0f ? ImVec4(0.35f, 1.0f, 0.45f, 1.0f) :
            (fps >= 45.0f ? ImVec4(1.0f, 0.82f, 0.28f, 1.0f) : ImVec4(1.0f, 0.35f, 0.28f, 1.0f));
        ImGui::TextColored(fpsColor, "Present %5.1f FPS  %5.2f ms",
                           fps, fps > 0.01f ? 1000.0f / fps : 0.0f);
        ImGui::Text("XR %5.1f Hz   VRCAM %5.1f FPS", g_perfMetrics.xrHz, g_perfMetrics.vrcamFps);
        ImGui::Text("Overlay pacing %s  previous %5.2f ms  slot %5.2f ms",
                    g_middleGroundPacingActive.load(std::memory_order_relaxed) ? "ON" : "starting",
                    g_previousOverlayWaitMs.load(std::memory_order_relaxed),
                    g_overlaySlotWaitMs.load(std::memory_order_relaxed));
        ImGui::TextDisabled("Present includes DLSS generated frames");
    }
    ImGui::End();
}
...
bool DrawLiveControls(LiveControlsUiState& state) {
    ...
    if (ImGui::BeginTabBar("CyberpunkVRPortTabs")) {
        ...
        ImGui::Checkbox("Performance overlay", &g_perfOverlayVisible);
...
bool WaitForOverlayFence(UINT64 targetValue, DWORD timeoutMs, const char* reason,
                         std::atomic<double>* waitMetric) {
    if (targetValue == 0) return true;
    if (!g_fence || !g_fenceEvent) return false;
    ...
    if (completed == UINT64_MAX) {
        ...
        Log("Overlay fence reported device removal while waiting for %s; overlay disabled.\n", reason);
        return false;
    }
    DWORD waitResult = WAIT_OBJECT_0;
    if (completed < targetValue) {
        const HRESULT eventHr = g_fence->SetEventOnCompletion(targetValue, g_fenceEvent);
        if (FAILED(eventHr)) {
            g_overlaySubmissionDisabled = true;
            Log("Overlay SetEventOnCompletion failed for %s (hr=0x%08X); overlay disabled.\n",
                reason, static_cast<unsigned>(eventHr));
            return false;
        }
        waitResult = WaitForSingleObject(g_fenceEvent, timeoutMs);
        completed = g_fence->GetCompletedValue();
    }

    QueryPerformanceCounter(&end);
    if (waitMetric && frequency.QuadPart > 0) {
        const double sampleMs = 1000.0 * static_cast<double>(end.QuadPart - start.QuadPart) /
            static_cast<double>(frequency.QuadPart);
        const double previous = waitMetric->load(std::memory_order_relaxed);
        waitMetric->store(previous <= 0.0 ? sampleMs : previous * 0.95 + sampleMs * 0.05,
                          std::memory_order_relaxed);
    }

    if (waitResult == WAIT_OBJECT_0 && completed != UINT64_MAX && completed >= targetValue) {
        return true;
    }
    if (waitResult == WAIT_FAILED) {
        g_overlaySubmissionDisabled = true;
        Log("Overlay fence wait failed for %s (win32=%lu); overlay disabled.\n",
            reason, static_cast<unsigned long>(GetLastError()));
        return false;
    }

    static std::atomic<UINT64> timeoutCount{0};
    const UINT64 count = timeoutCount.fetch_add(1, std::memory_order_relaxed) + 1;
    if (count == 1 || (count % 60) == 0) {
        Log("Overlay fence wait timed out for %s (target=%llu completed=%llu count=%llu); "
            "skipping this overlay frame.\n",
            reason,
            static_cast<unsigned long long>(targetValue),
            static_cast<unsigned long long>(completed),
            static_cast<unsigned long long>(count));
    }
    if (completed == UINT64_MAX) g_overlaySubmissionDisabled = true;
    return false;
}

bool DrainOverlayGpu(DWORD timeoutMs, const char* reason) {
    if (!g_queue || !g_fence || !g_fenceEvent) return true;
    const UINT64 targetValue = ++g_fenceValue;
    const HRESULT signalHr = g_queue->Signal(g_fence, targetValue);
    if (FAILED(signalHr)) {
        g_overlaySubmissionDisabled = true;
        Log("Overlay drain signal failed for %s (hr=0x%08X).\n",
            reason, static_cast<unsigned>(signalHr));
        return false;
    }
    return WaitForOverlayFence(targetValue, timeoutMs, reason, nullptr);
}

bool WaitForPreviousOverlaySubmission() {
    const UINT64 targetValue = g_previousOverlayFenceValue;
    if (targetValue == 0) {
        g_middleGroundPacingActive.store(true, std::memory_order_relaxed);
        return true;
    }
    const bool ready = WaitForOverlayFence(targetValue, 100, "previous-overlay throttle",
...
bool WaitForFrameSlot(const FrameContext& frame) {
    UINT64 targetValue = frame.fenceValue;
    if (!g_imguiFrameFenceValues.empty()) {
        const UINT slot = g_nextImGuiFrameSlot % static_cast<UINT>(g_imguiFrameFenceValues.size());
        targetValue = std::max(targetValue, g_imguiFrameFenceValues[slot]);
    }
    return WaitForOverlayFence(targetValue, 100, "frame-slot reuse", &g_overlaySlotWaitMs);
}

void ReleaseRenderTargets() {
    // Normal frames are nonblocking, so teardown/resize is the one place that must drain before
    // releasing backbuffers, allocators, the command list, and ImGui upload resources.
    if (g_queue && g_fence && (g_cmdList || !g_frames.empty())) {
        if (!DrainOverlayGpu(2000, "resource teardown")) {
            Log("Overlay teardown drain timed out or failed; continuing (device may be removed).\n");
        }
    }
    ...
    g_frames.clear();
    std::fill(g_imguiFrameFenceValues.begin(), g_imguiFrameFenceValues.end(), 0);
    g_nextImGuiFrameSlot = 0;
    g_previousOverlayFenceValue = 0;
    g_middleGroundPacingActive.store(false, std::memory_order_relaxed);
    g_previousOverlayWaitMs.store(0.0, std::memory_order_relaxed);
    g_overlaySlotWaitMs.store(0.0, std::memory_order_relaxed);
...
bool EnsureSwapchainResources(IDXGISwapChain* swapChain) {
    ...
    const bool needsResources = !g_swapChain3 || g_swapChain3 != swapChain3 ||
        g_frameCount != desc.BufferCount || g_rtvFormat != desc.BufferDesc.Format;
    if (!needsResources) {
        swapChain3->Release();
        return true;
    }
...
bool EnsureImGui(IDXGISwapChain* swapChain) {
    ...
    if (!g_imguiInitialized) {
        ...
        g_imguiFrameFenceValues.assign(std::max<UINT>(desc.BufferCount, 2), 0);
        g_nextImGuiFrameSlot = 0;
        g_overlaySubmissionDisabled = false;
...
void OverlayRender(IDXGISwapChain* swapChain) {
    // Streamline rotates Present over a worker pool. Mode 2 must not be bypassed by a try-lock:
    // every overlay submission has to observe the previous submission's fence before it can add
    // more work to the shared queue. Hold ownership through recording and signaling so another
    // Present worker cannot pass the check before this frame publishes its fence value.
    std::lock_guard<std::mutex> lock(g_overlayMutex);
    if (g_overlaySubmissionDisabled) return;
    if (!EnsureImGui(swapChain)) return;
    if (!WaitForPreviousOverlaySubmission()) return;
    ...
    const UINT imguiSlot = g_imguiFrameFenceValues.empty()
        ? 0
        : g_nextImGuiFrameSlot % static_cast<UINT>(g_imguiFrameFenceValues.size());
    ImGui_ImplDX12_RenderDrawData(ImGui::GetDrawData(), g_cmdList);
    if (!g_imguiFrameFenceValues.empty()) {
        g_nextImGuiFrameSlot = (imguiSlot + 1) % static_cast<UINT>(g_imguiFrameFenceValues.size());
    }
    ...
    if (FAILED(g_cmdList->Close())) {
        g_overlaySubmissionDisabled = true;
        Log("Overlay command list close failed; overlay disabled.\n");
        return;
    }
    ID3D12CommandList* lists[] = {g_cmdList};
    g_queue->ExecuteCommandLists(1, lists);

    // Stamp without draining the just-submitted frame. The next OverlayRender waits this exact
    // value before recording again (Mode 2), while the allocator and ImGui rings retain their own
    // exact reuse stamps. The shared-queue signal therefore caps runahead at one overlay frame.
    const UINT64 submissionFence = ++g_fenceValue;
    const HRESULT signalHr = g_queue->Signal(g_fence, submissionFence);
    if (SUCCEEDED(signalHr)) {
        frame.fenceValue = submissionFence;
