...
DWORD WINAPI HookedXInputGetState(DWORD dwUserIndex, XINPUT_STATE* pState) {
    ...
    bool scannerHold = false;
    {
        ...
        if (CyberpunkVR_ScannerGesture != 0 && !OpenXRManager::Get().IsRuntimePsvr2()) {
            ...
            // The `!g_isInVehicle` is this fork's: PSVR2 left-grip scanner synthesis is
            // suppressed for every mounted state (.okf/architecture/vehicle-interaction-pipeline.md).
            s_gesture = s_earArmed && gripHeld && (g_menuModeValue == 0) && !g_isInVehicle;
        } else {
            s_earArmed = false;
            s_inMs = 0.0;
            s_outMs = 0.0;
